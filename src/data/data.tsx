export const dataDynamics = [
    {
      "skills": "Hard Skills",
      "kvartal": "1 квартал",
      "assessment": 2
    },
    {
      "skills": "Hard Skills",
      "kvartal": "2 квартал",
      "assessment": 3
    },
    {
      "skills": "Hard Skills",
      "kvartal": "3 квартал",
      "assessment": 4
    },
    {
      "skills": "Hard Skills",
      "kvartal": "4 квартал",
      "assessment": 4
    },
    {
      "skills": "Soft Skills",
      "kvartal": "1 квартал",
      "assessment": 3
    },
    {
      "skills": "Soft Skills",
      "kvartal": "2 квартал",
      "assessment": 4
    },
    {
      "skills": "Soft Skills",
      "kvartal": "3 квартал",
      "assessment": 4
    },
    {
      "skills": "Soft Skills",
      "kvartal": "4 квартал",
      "assessment": 4
    }
];

export const dataStress = [
    { квартал: '1 кв', оценка: 0.5 },
    { квартал: '2 кв', оценка: 1 },
    { квартал: '3 кв', оценка: 1.5 },
    { квартал: '4 кв', оценка: 2 }
]

export const dataSkillsAssessment = [
  {
    "skills": "Hard Skills",
    "assessment": 4,
    "person": "Акимов Роберт"
  },
  {
    "skills": "Soft Skills",
    "assessment": 4,
    "person": "Акимов Роберт"
  },

  {
    "skills": "Hard Skills",
    "assessment": 2,
    "person": "Волков Денис"
  },
  {
    "skills": "Soft Skills",
    "assessment": 3,
    "person": "Волков Денис"
  },
  
  {
    "skills": "Hard Skills",
    "assessment": 4,
    "person": "Журавлев Максим"
  },
  {
    "skills": "Soft Skills",
    "assessment": 2,
    "person": "Журавлев Максим"
  },

  {
    "skills": "Hard Skills",
    "assessment": 4,
    "person": "Комарова Вера"
  },
  {
    "skills": "Soft Skills",
    "assessment": 3,
    "person": "Комарова Вера"
  },

  {
    "skills": "Hard Skills",
    "assessment": 4,
    "person": "Костин Михаил"
  },
  {
    "skills": "Soft Skills",
    "assessment": 4,
    "person": "Костин Михаил"
  },
];


export const dataHard = [
  {
    "skills": "Hard Skills",
    "kvartal": "1 квартал",
    "assessment": 2
  },
  {
    "skills": "Hard Skills",
    "kvartal": "2 квартал",
    "assessment": 3
  },
  {
    "skills": "Hard Skills",
    "kvartal": "3 квартал",
    "assessment": 4
  },
  {
    "skills": "Hard Skills",
    "kvartal": "4 квартал",
    "assessment": 0
  }
];

export const dataSoft = [
  {
    "skills": "Soft Skills",
    "kvartal": "1 квартал",
    "assessment": 3
  },
  {
    "skills": "Soft Skills",
    "kvartal": "2 квартал",
    "assessment": 4
  },
  {
    "skills": "Soft Skills",
    "kvartal": "3 квартал",
    "assessment": 4
  },
  {
    "skills": "Soft Skills",
    "kvartal": "4 квартал",
    "assessment": 0
  }
];

export const dataCompliance = [
  {
    "skills": "Коэффициент соответствия",
    "kvartal": "1 квартал",
    "assessment": 0.4
  },
  {
    "skills": "Коэффициент соответствия",
    "kvartal": "2 квартал",
    "assessment": 0.6
  },
  {
    "skills": "Коэффициент соответствия",
    "kvartal": "3 квартал",
    "assessment": 0.8
  },
  {
    "skills": "Коэффициент соответствия",
    "kvartal": "4 квартал",
    "assessment": 0
  }
];

export const dataEmployeeAssessment = [
  {
    "skills": "Текущая оценка",
    "assessment": 3,
    "person": "Asana"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 4,
    "person": "Asana"
  },
  {
    "skills": "Текущая оценка",
    "assessment": 2,
    "person": "Clickhouse"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 3,
    "person": "Clickhouse"
  },
  {
    "skills": "Текущая оценка",
    "assessment": 3,
    "person": "3D-анимация"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 4,
    "person": "3D-анимация"
  },
  {
    "skills": "Текущая оценка",
    "assessment": 1,
    "person": "Java"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 3,
    "person": "Java"
  },  
  {
    "skills": "Текущая оценка",
    "assessment": 1,
    "person": "С++"
  }, 
  {
    "skills": "Минимальная оценка",
    "assessment": 1,
    "person": "С++"
  },
  {
    "skills": "Текущая оценка",
    "assessment": 4,
    "person": "Photoshop"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 3,
    "person": "Photoshop"
  },
  {
    "skills": "Текущая оценка",
    "assessment": 5,
    "person": "Trello"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 4,
    "person": "Trello"
  },
  {
    "skills": "Текущая оценка",
    "assessment": 4,
    "person": "Английский язык"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 3,
    "person": "Английский язык"
  },
  {
    "skills": "Текущая оценка",
    "assessment": 1,
    "person": "Китайский язык"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 1,
    "person": "Китайский язык"
  },
  {
    "skills": "Текущая оценка",
    "assessment": 4,
    "person": "Креативное мышление"
  },
  {
    "skills": "Минимальная оценка",
    "assessment": 3,
    "person": "Креативное мышление"
  },
];